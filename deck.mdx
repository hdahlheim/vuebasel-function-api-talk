import {
  CodeSurferLayout,
} from "code-surfer"
import custom from './theme'
import { Appear, Notes, Image, Head } from 'mdx-deck'
import { Split } from 'mdx-deck/layouts'
import { oceanicNextPartial } from "code-surfer"
import ThreeCGrid from './ThreeCGrid'

export const themes = [ custom, oceanicNextPartial ]


<Head>
  <title>Vue function API</title>
</Head>


<h1>
A look at the
<br/>
Function API RFC
</h1>

---

# Outline

- Introduction
- Vue 3
- The RFC (or why everyone got mad)
- The RFC in detail
- Code Examples
- Q&A

---

# Almost everything in this talk is subject to change

---

<div style={{textAlign: 'left'}}>
<Appear>
<h1 style={{margin: '1rem 0'}}>Hi 👋,</h1>
<h1 style={{margin: '1rem 0'}}>I’m Henning</h1>
<div>
<strong>
Software developer apprentice <span style={{color: '#FF8000'}}>@cyon.</span>
<br/>
Worked with Vue.js for over a year.
</strong>
</div>
</Appear>
</div>

<Notes>

This is a note

</Notes>

---

<h1>
Vue <span style={{color: '#48bb78'}}>3</span>
</h1>

---

# The RFC

---

<Image src="./img/therfc.png" size="80%" />

---

<Image src="./img/damdamdam.png" size="70%" />

---

# 🔥🔥🔥

---

<Image src="./img/a2v3.png" size="60%" />

---

<Image src="./img/lookslikereact.jpg" size="50%" />

---
<!--
# So what about React?

<Notes>

So lats talk a bit about react
because react actually went through
almost the same change just a few
months prior

</Notes>

---

# ⚛️ React Hooks

---

<CodeSurferLayout>

```jsx title="React Component without hooks"
import React from 'react'

class MyComponent extends React.Component {
  constructor(props) {
    super(props)
    this.state = { count: 0 }
    this.increment = this.increment.bind(this)
  }

  increment () {
    this.setState({ count: this.state.count + 1 })
  }

  render () {
    return (
      <div>
        <p>You clicked {this.state.count} times</p>
        <button
          onClick={this.increment}
        >Click me</button>
      </div>
    )
  }
}

export default MyComponent
```

```jsx 4:12
import React from 'react'

class MyComponent extends React.Component {
  constructor(props) {
    super(props)
    this.state = { count: 0 }
    this.increment = this.increment.bind(this)
  }

  increment () {
    this.setState({ count: this.state.count + 1 })
  }

  render () {
    return (
      <div>
        <p>You clicked {this.state.count} times</p>
        <button
          onClick={this.increment}
        >Click me</button>
      </div>
    )
  }
}

export default MyComponent
```

```jsx 12:22
import React from 'react'

class MyComponent extends React.Component {
  constructor(props) {
    super(props)
    this.state = { count: 0 }
    this.increment = this.increment.bind(this)
  }

  increment () {
    this.setState({ count: this.state.count + 1 })
  }

  render () {
    return (
      <div>
        <p>You clicked {this.state.count} times</p>
        <button
          onClick={this.increment}
        >Click me</button>
      </div>
    )
  }
}

export default MyComponent
```

```jsx 1:24
import React from 'react'

class MyComponent extends React.Component {
  constructor(props) {
    super(props)
    this.state = { count: 0 }
    this.increment = this.increment.bind(this)
  }

  increment () {
    this.setState({ count: this.state.count + 1 })
  }

  render () {
    return (
      <div>
        <p>You clicked {this.state.count} times</p>
        <button
          onClick={this.increment}
        >Click me</button>
      </div>
    )
  }
}

export default MyComponent
```

</CodeSurferLayout>

<Notes>

So for those unfamiliar with react

this was for the longest time how you write a statefull react component

using the ES2015 class syntax

**this**

</Notes>


---

<CodeSurferLayout>

```jsx title="React component with hooks"
import React, { useState } from 'react'

function MyComponent () {
  const [count, setCount] = useState(0)
  const increment = () => setCount(count + 1)

  return (
    <div>
      <p>You clicked {count} times</p>
      <button onClick={increment}>
        Click me
      </button>
    </div>
  )
}

export default MyComponent
```

```js 1,4:5
import React, { useState } from 'react'

function MyComponent () {
  const [count, setCount] = useState(0)
  const increment = () => setCount(count + 1)

  return (
    <div>
      <p>You clicked {count} times</p>
      <button onClick={increment}>
        Click me
      </button>
    </div>
  )
}

export default MyComponent
```

```js 7:14
import React, { useState } from 'react'

function MyComponent () {
  const [count, setCount] = useState(0)
  const increment = () => setCount(count + 1)

  return (
    <div>
      <p>You clicked {count} times</p>
      <button onClick={increment}>
        Click me
      </button>
    </div>
  )
}

export default MyComponent
```

```js 1:18
import React, { useState } from 'react'

function MyComponent () {
  const [count, setCount] = useState(0)
  const increment = () => setCount(count + 1)

  return (
    <div>
      <p>You clicked {count} times</p>
      <button onClick={increment}>
        Click me
      </button>
    </div>
  )
}

export default MyComponent
```

</CodeSurferLayout>

<Notes>

So this how you write a
react component with hooks

you just useState and be done with it

the reason I’m showing you this is
because when hooks were introduced
the react community was quite happy

but it also did not come with a deprecation of the old api ;) so this might have something todo with it

</Notes>

--- -->

## The function API
<ul>
<Appear>
<li>
🎉 Purely additive.
<span style={{ fontSize: '50%' }}> (The current API will not be replaced)</span>
</li>
<li>
💪 It will work side by side with the current API.
</li>
<li>
🤯 Even in the same component.
</li>
</Appear>
</ul>

---

## What do we get?
<Appear>

**a new `setup()` method and composition functions**

</Appear>

---


<CodeSurferLayout>

```js title="setup()"
export default {
  setup () {
    return {
      // ...
    }
  }
}
```

```js 1:7 subtitle="everything returned from 'setup' is available in render"
export default {
  setup () {
    return {
      // ...
    }
  }
}
```

```js 1:7 subtitle="'this' is not availably in setup"
export default {
  setup () {
    return {
      // ...
    }
  }
}
```


```js 1:7 subtitle="'setup' takes two arguments"
export default {
  setup (props, context) {
    return {
      // ...
    }
  }
}
```

```js 1:7 subtitle="'props' gives access to all props defined on the component"
export default {
  props:{
    msg: String
  },
  setup (props, context) {
    console.log(props.msg)
  }
}
```

```js 3:8 subtitle="'context' exposes everything else"
export default {
  setup (props, context) {
    // context.attrs
    // context.slots
    // context.refs
    // context.emit
    // context.parent
    // context.root
  }
}
```

</CodeSurferLayout>

<Notes>

Setup gets called first after all props are resolved

</Notes>

---

## The composition functions

<Notes>

inside of setup you can use functions

</Notes>


---

<ThreeCGrid>
<code>value</code>
<code>state</code>
<code>computed</code>
<code>watch</code>
<code>provide</code>
<code>inject</code>
<code>onActivated</code>
<code>onDeactivated</code>
<code>onBeforeMount</code>
<code>onBeforeUpdate</code>
<code>onCreated</code>
<code>onUpdated</code>
<code>onUnmounted</code>
<code>onDestroyed</code>
<code>onBeforeDestroy</code>
<code>createComponent</code>
<code>createElement</code>
</ThreeCGrid>

<Notes>

These are all of the functions added to vue
or at least all that i was able to find are reference to

**value** **state** **onXXX** and **createComponent** **createElement**
</Notes>

---


## Why all of this

<ul>
<Appear>
<li >
🧩 Improved logic reuse
</li>
<li >
🛠 Better TypeScript support
</li>
<li >
🗜 Smaller bundle size
</li>
</Appear>
</ul>

---

<CodeSurferLayout>

```jsx title="Vue 2 component"
<template>
  <div>
    <p>You clicked {{ count }} times</p>
    <button @click="increment">Click me</button>
  </div>
</template>

<script>
export default {
  data () {
    return {
      count: 0
    }
  },
  methods:{
    increment () {
      this.count++
    }
  }
}
</script>
```

```jsx 10:14
<template>
  <div>
    <p>You clicked {{ count }} times</p>
    <button @click="increment">Click me</button>
  </div>
</template>

<script>
export default {
  data () {
    return {
      count: 0
    }
  },
  methods:{
    increment () {
      this.count++
    }
  }
}
</script>
```

```jsx 14:18
<template>
  <div>
    <p>You clicked {{ count }} times</p>
    <button @click="increment">Click me</button>
  </div>
</template>

<script>
export default {
  data () {
    return {
      count: 0
    }
  },
  methods:{
    increment () {
      this.count++
    }
  }
}
</script>
```

```jsx 1:22
<template>
  <div>
    <p>You clicked {{ count }} times</p>
    <button @click="increment">Click me</button>
  </div>
</template>

<script>
export default {
  data () {
    return {
      count: 0
    }
  },
  methods:{
    increment () {
      this.count++
    }
  }
}
</script>
```

```jsx 1:21 title="Vue 3 function API"
<template>
  <div>
    <p>You clicked {{ count }} times</p>
    <button @click="increment">Click me</button>
  </div>
</template>

<script>
import { value } from 'vue'

export default {
  setup () {
    const count = value(0)
    const increment = () => count.value++
    return {
      count,
      increment
    }
  }
}
</script>
```

```jsx 9
<template>
  <div>
    <p>You clicked {{ count }} times</p>
    <button @click="increment">Click me</button>
  </div>
</template>

<script>
import { value } from 'vue'

export default {
  setup () {
    const count = value(0)
    const increment = () => count.value++
    return {
      count,
      increment
    }
  }
}
</script>
```

```jsx 12:14
<template>
  <div>
    <p>You clicked {{ count }} times</p>
    <button @click="increment">Click me</button>
  </div>
</template>

<script>
import { value } from 'vue'

export default {
  setup () {
    const count = value(0)
    const increment = () => count.value++
    return {
      count,
      increment
    }
  }
}
</script>
```

```jsx 15:18
<template>
  <div>
    <p>You clicked {{ count }} times</p>
    <button @click="increment">Click me</button>
  </div>
</template>

<script>
import { value } from 'vue'

export default {
  setup () {
    const count = value(0)
    const increment = () => count.value++
    return {
      count,
      increment
    }
  }
}
</script>
```

```jsx 1:21
<template>
  <div>
    <p>You clicked {{ count }} times</p>
    <button @click="increment">Click me</button>
  </div>
</template>

<script>
import { value } from 'vue'

export default {
  setup () {
    const count = value(0)
    const increment = () => count.value++
    return {
      count,
      increment
    }
  }
}
</script>
```

</CodeSurferLayout>

<Notes>

constants value

update the value of this constant by incrementing it by one

the reason for this is because javascript dose not pass
references to primitives like Numbers or Strings so they get wrapped
and are accessed using .value

</Notes>

---

## Not all components are this simple

---

## What about lifecycles

---

<CodeSurferLayout>

```jsx
<template>
  <div>Mouse is at {{ x }}, {{ y }}</div>
</template>

<script>
export default {
  data () {
    return {
      x: 0,
      y: 0
    }
  },
  mounted () {
    window.addEventListener('mousemove', this.updateMouse)
  },
  destroyed () {
    window.removeEventListener('mousemove', this.updateMouse)
  },
  methods: {
    updateMouse (e) {
      this.x = e.pageX
      this.y = e.pageY
    }
  }
}
</script>
```

```jsx 13:18
<template>
  <div>Mouse is at {{ x }}, {{ y }}</div>
</template>

<script>
export default {
  data () {
    return {
      x: 0,
      y: 0
    }
  },
  mounted () {
    window.addEventListener('mousemove', this.updateMouse)
  },
  destroyed () {
    window.removeEventListener('mousemove', this.updateMouse)
  },
  methods: {
    updateMouse (e) {
      this.x = e.pageX
      this.y = e.pageY
    }
  }
}
</script>
```

```jsx 19:24
<template>
  <div>Mouse is at {{ x }}, {{ y }}</div>
</template>

<script>
export default {
  data () {
    return {
      x: 0,
      y: 0
    }
  },
  mounted () {
    window.addEventListener('mousemove', this.updateMouse)
  },
  destroyed () {
    window.removeEventListener('mousemove', this.updateMouse)
  },
  methods: {
    updateMouse (e) {
      this.x = e.pageX
      this.y = e.pageY
    }
  }
}
</script>
```

```jsx 1:26
<template>
  <div>Mouse is at {{ x }}, {{ y }}</div>
</template>

<script>
export default {
  data () {
    return {
      x: 0,
      y: 0
    }
  },
  mounted () {
    window.addEventListener('mousemove', this.updateMouse)
  },
  destroyed () {
    window.removeEventListener('mousemove', this.updateMouse)
  },
  methods: {
    updateMouse (e) {
      this.x = e.pageX
      this.y = e.pageY
    }
  }
}
</script>
```

```jsx 1:28
<template>
  <div>Mouse is at {{ x }}, {{ y }}</div>
</template>

<script>
import { value } from 'vue'

export default {
  data () {
    return {
      x: 0,
      y: 0
    }
  },
  mounted () {
    window.addEventListener('mousemove', this.updateMouse)
  },
  destroyed () {
    window.removeEventListener('mousemove', this.updateMouse)
  },
  methods: {
    updateMouse (e) {
      this.x = e.pageX
      this.y = e.pageY
    }
  }
}
</script>
```

```jsx 1:30
<template>
  <div>Mouse is at {{ x }}, {{ y }}</div>
</template>

<script>
import { value } from 'vue'

export default {
  setup () {
    const x = value(0)
    const y = value(0)
    return {
      x,
      y
    }
  },
  mounted () {
    window.addEventListener('mousemove', this.updateMouse)
  },
  destroyed () {
    window.removeEventListener('mousemove', this.updateMouse)
  },
  methods: {
    updateMouse (e) {
      this.x = e.pageX
      this.y = e.pageY
    }
  }
}
</script>
```

```jsx 1:31
<template>
  <div>Mouse is at {{ x }}, {{ y }}</div>
</template>

<script>
import { value } from 'vue'

export default {
  setup () {
    const x = value(0)
    const y = value(0)

    const updateMouse = (e) => {
      x.value = e.pageX
      y.value = e.pageY
    }

    return {
      x,
      y,
      updateMouse
    }
  },
  mounted () {
    window.addEventListener('mousemove', this.updateMouse)
  },
  destroyed () {
    window.removeEventListener('mousemove', this.updateMouse)
  }
}
</script>
```

```jsx 1:32
<template>
  <div>Mouse is at {{ x }}, {{ y }}</div>
</template>

<script>
import { value, onMounted } from 'vue'

export default {
  setup () {
    const x = value(0)
    const y = value(0)

    const updateMouse = (e) => {
      x.value = e.pageX
      y.value = e.pageY
    }

    return {
      x,
      y,
      updateMouse
    }
  },
  mounted () {
    window.addEventListener('mousemove', this.updateMouse)
  },
  destroyed () {
    window.removeEventListener('mousemove', this.updateMouse)
  }
}
</script>
```

```jsx 1:32
<template>
  <div>Mouse is at {{ x }}, {{ y }}</div>
</template>

<script>
import { value, onMounted } from 'vue'

export default {
  setup () {
    const x = value(0)
    const y = value(0)

    const updateMouse = (e) => {
      x.value = e.pageX
      y.value = e.pageY
    }

    onMounted(() => {
      window.addEventListener('mousemove', updateMouse)
    })

    return {
      x,
      y,
      updateMouse
    }
  },
  destroyed () {
    window.removeEventListener('mousemove', this.updateMouse)
  }
}
</script>
```

```jsx 1:32
<template>
  <div>Mouse is at {{ x }}, {{ y }}</div>
</template>

<script>
import { value, onMounted, onUnmounted } from 'vue'

export default {
  setup () {
    const x = value(0)
    const y = value(0)

    const updateMouse = (e) => {
      x.value = e.pageX
      y.value = e.pageY
    }

    onMounted(() => {
      window.addEventListener('mousemove', updateMouse)
    })

    return {
      x,
      y,
      updateMouse
    }
  },
  destroyed () {
    window.removeEventListener('mousemove', this.updateMouse)
  }
}
</script>
```

```jsx 1:32
<template>
  <div>Mouse is at {{ x }}, {{ y }}</div>
</template>

<script>
import { value, onMounted, onUnmounted } from 'vue'

export default {
  setup () {
    const x = value(0)
    const y = value(0)

    const updateMouse = (e) => {
      x.value = e.pageX
      y.value = e.pageY
    }

    onMounted(() => {
      window.addEventListener('mousemove', updateMouse)
    })

    onUnmounted(() => {
      window.removeEventListener('mousemove', updateMouse)
    })

    return {
      x,
      y,
      updateMouse
    }
  }
}
</script>
```

```jsx 1:32
<template>
  <div>Mouse is at {{ x }}, {{ y }}</div>
</template>

<script>
import { value, onMounted, onUnmounted } from 'vue'

export default {
  setup () {
    const x = value(0)
    const y = value(0)

    const updateMouse = (e) => {
      x.value = e.pageX
      y.value = e.pageY
    }

    onMounted(() => {
      window.addEventListener('mousemove', updateMouse)
    })

    onUnmounted(() => {
      window.removeEventListener('mousemove', updateMouse)
    })

    return {
      x,
      y
    }
  }
}
</script>
```

</CodeSurferLayout>

---

# Extracting Logic

---

<CodeSurferLayout>

```jsx 1:32
<template>
  <div>Mouse is at {{ x }}, {{ y }}</div>
</template>

<script>
import { value, onMounted, onUnmounted } from 'vue'

export default {
  setup () {
    const x = value(0)
    const y = value(0)

    const updateMouse = (e) => {
      x.value = e.pageX
      y.value = e.pageY
    }

    onMounted(() => {
      window.addEventListener('mousemove', updateMouse)
    })

    onUnmounted(() => {
      window.removeEventListener('mousemove', updateMouse)
    })

    return {
      x,
      y
    }
  }
}
</script>
```

```jsx 10:24
<template>
  <div>Mouse is at {{ x }}, {{ y }}</div>
</template>

<script>
import { value, onMounted, onUnmounted } from 'vue'

export default {
  setup () {
    const x = value(0)
    const y = value(0)

    const updateMouse = (e) => {
      x.value = e.pageX
      y.value = e.pageY
    }

    onMounted(() => {
      window.addEventListener('mousemove', updateMouse)
    })

    onUnmounted(() => {
      window.removeEventListener('mousemove', updateMouse)
    })
    return {
      x,
      y
    }
  }
}
</script>
```

```jsx
<template>
  <div>Mouse is at {{ x }}, {{ y }}</div>
</template>

<script>
import { value, onMounted, onUnmounted } from 'vue'

function useMousePosition () {
  const x = value(0)
  const y = value(0)

  const updateMouse = (e) => {
    x.value = e.pageX
    y.value = e.pageY
  }

  onMounted(() => {
    window.addEventListener('mousemove', updateMouse)
  })

  onUnmounted(() => {
    window.removeEventListener('mousemove', updateMouse)
  })

  return { x, y }
}

export default {
  setup () {
    return {
      ...useMousePosition()
    }
  }
}
</script>
```

```jsx 1:32
<template>
  <div>Mouse is at {{ x }}, {{ y }}</div>
</template>

<script>
import { value, onMounted, onUnmounted } from 'vue'

function useMousePosition () {
  const x = value(0)
  const y = value(0)
  const updateMouse = (e) => {
    x.value = e.pageX
    y.value = e.pageY
  }
  onMounted(() => {
    window.addEventListener('mousemove', updateMouse)
  })
  onUnmounted(() => {
    window.removeEventListener('mousemove', updateMouse)
  })
  return { x, y }
}

export default {
  setup () {
    return {
      ...useMousePosition()
    }
  }
}
</script>
```

</CodeSurferLayout>

<Notes>

Nothing is tied to the component

</Notes>

---

<div style={{textAlign: 'left'}}>
<h1>
<Appear>
<span>Henning,<br/></span>
<span>that’s cool and all</span>
<span><br/>but<br/></span>
<span>I want this now!!</span>
</Appear>
</h1>
</div>

---

<CodeSurferLayout>

```bash
npm i -s vue-function-api
```

</CodeSurferLayout>

---

<CodeSurferLayout>

```js
import { plugin } from 'vue-function-api'

Vue.use(plugin)
```

</CodeSurferLayout>

---

<CodeSurferLayout>

```jsx 1:22
<template>
  <div>
    <p>You clicked {{ count }} times</p>
    <button @click="increment">Click me</button>
  </div>
</template>

<script>
import { value } from 'vue'

export default {
  setup () {
    const count = value(0)
    const increment = () => count.value++
    return {
      count,
      increment
    }
  }
}
</script>
```

```jsx 1:22
<template>
  <div>
    <p>You clicked {{ count }} times</p>
    <button @click="increment">Click me</button>
  </div>
</template>

<script>
import { value } from 'vue-function-api'

export default {
  setup () {
    const count = value(0)
    const increment = () => count.value++
    return {
      count,
      increment
    }
  }
}
</script>
```

</CodeSurferLayout>


---

# Thanks 🙏

---

# Questions?
